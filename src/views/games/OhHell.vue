				...
				<v-dialog v-model="playerDialog" max-width="500px">
					<template v-slot:activator="{ on }">
						<div class="d-flex justify-content-center align-center">
							<v-btn large tile v-on="on"><v-icon dark class="mr-2">mdi-account-plus-outline</v-icon>Add Player</v-btn>
						</div>
					</template>
					<v-card>
						<v-card-title>
        ...
        <v-row class="pa-4 pl-11">
            <div class="table">
                ...
                <v-toolbar flat color="white">
                    ...
                    <template v-slot:activator="{ on }">
                        <v-btn light class="mr-n2" v-on="on" icon title="Add Round"><v-icon dark>mdi-plus-circle-outline</v-icon></v-btn>
                    </template>
                    <v-card>
                        <v-card-title>
					...
					<v-btn light icon title="Remove Player"><v-icon dark>mdi-account-minus-outline</v-icon></v-btn>
...
export default {
    data: () => ({
        ...
        deleteItem (item) {
            ...
            confirm('Are you sure you want to delete this item?') && this.rounds.splice(index, 1)
        },
